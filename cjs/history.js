...
function createBrowserHistory$1(props) {
  ...
  function setState(nextState) {
    ...
    var index = queue.findIndex(function (q) {
      return q.location.pathname == history.location.pathname && q.location.search == history.location.search && q.location.hash == history.location.hash;
    });
    if (index != -1) queue.splice(index);
    queue.push({
  ...
  function next() {
    if (queue.length) {
      var _queue$shift = queue.shift(),
          location = _queue$shift.location,
          action = _queue$shift.action;

      transitionManager.notifyListeners(location, action);
    }
  }
...
exports.createBrowserTransitionHistory = createBrowserHistory$1;
